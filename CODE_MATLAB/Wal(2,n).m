...
W_3 = ones(size(t));   % Khởi tạo một vector chứa toàn bộ giá trị +1

% Thay đổi giá trị thành -1 theo các đoạn thời gian sao cho giá trị lần lượt là 1, -1, -1, 1 cách đều
W_3(t >= -4 & t < -2) = 1;   % Thay đổi giá trị thành +1 trong khoảng từ -4 đến -2
W_3(t >= -2 & t < 0) = -1;   % Thay đổi giá trị thành -1 trong khoảng từ -2 đến 0
W_3(t >= 0 & t < 2) = -1;    % Thay đổi giá trị thành -1 trong khoảng từ 0 đến 2
W_3(t >= 2 & t < 4) = 1;     % Thay đổi giá trị thành +1 trong khoảng từ 2 đến 4
...
W_3_fft = fft(W_3);  % Phép biến đổi Fourier
...
% Tính và vẽ phổ pha
W_3_phase = angle(W_3_fft);  % Phổ pha

% Lấy một nửa phổ pha
W_3_phase_half = W_3_phase(1:N/2+1);

subplot(3,1,3);
plot(f, W_3_phase_half, 'LineWidth', 2);
